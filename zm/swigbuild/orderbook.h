...
class Orderbook {
    ...
    public:
        ...
        vector<pair<float, Limit>> ask_map(){
            vector<pair<float, Limit>> v;
            v.resize(asks.size());
            copy(asks.begin(), asks.end(), v.begin());
            return v;
        }

        vector<pair<float, Limit>> bid_map(){
            vector<pair<float, Limit>> v;
            v.resize(bids.size());
            copy(bids.begin(), bids.end(), v.begin());
            return v;
        }
